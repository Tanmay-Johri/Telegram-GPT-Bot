...
def attn(x, scope, n_state, *, past, hparams):
    ...
    def split_heads(x):
        ...
        return tf.transpose(a=split_states(x, 12), perm=[0, 2, 1, 3])
...
def past_shape(*, hparams, batch_size=None, sequence=None):
    return [batch_size, 12, 2, 12, sequence, 768 // 12]
...
def model(hparams, X, past=None, scope='model', reuse=False):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        wpe = tf.compat.v1.get_variable('wpe', [1024, 768],
                             initializer=tf.compat.v1.random_normal_initializer(stddev=0.01))
        wte = tf.compat.v1.get_variable('wte', [0, 768],
        ...
        # Transformer
        presents = []
        pasts = tf.unstack(past, axis=1) if past is not None else [None] * 12
        assert len(pasts) == 12
        ...
        h_flat = tf.reshape(h, [batch*sequence, 768])
        logits = tf.matmul(h_flat, wte, transpose_b=True)
        logits = tf.reshape(logits, [batch, sequence, 0])
