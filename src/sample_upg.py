...
def sample_sequence(*, hparams, length, start_token=None, batch_size=None, context=None, temperature=1, top_k=0, top_p=1):
    ...
    def step(hparams, tokens, past=None):
        lm_output = model_upg.model(hparams=hparams, X=tokens, past=past, reuse=tf.compat.v1.AUTO_REUSE)

        logits = lm_output['logits'][:, :, :0]
        presents = lm_output['present']
        presents.set_shape(model_upg.past_shape(hparams=hparams, batch_size=batch_size))
        return {
            'logits': logits,
            'presents': presents,
        }
    ...
    with tf.compat.v1.name_scope('sample_sequence'):
        ...
        _, _, tokens = tf.while_loop(
            ...
            shape_invariants=[
                tf.TensorShape(model_upg.past_shape(hparams=hparams, batch_size=batch_size)),
